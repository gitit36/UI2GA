...
const App: React.FC = () => {
  ...
  const handleDeleteRow = useCallback((itemNo: number) => {
      if (!activeScreenshotId) return;
      setResults(prev => {
        ...
        return { ...prev, [activeScreenshotId]: { events: newEvents, annotations: newAnns } };
      });
  }, [activeScreenshotId]);
  ...
  const handleUpdateEvents = useCallback((events: TaggingEvent[]) => {
    ...
    setResults(prev => {
      const current = prev[activeScreenshotId];
      if (!current) return prev;
      return { ...prev, [activeScreenshotId]: { ...current, events: events } };
    });
  }, [activeScreenshotId]);
  ...
  const handleAddRow = useCallback(() => {
      if (!activeScreenshotId) return;
      setResults(prev => {
        const current = prev[activeScreenshotId] || { events: [], annotations: [] };
        ...
        return { ...prev, [activeScreenshotId]: { ...current, events: [...current.events, newEvent] } };
      });
  }, [activeScreenshotId, language]);
  ...
  const handleAddAnnotation = (rect: SelectionRect) => {
       ...
       setResults(prev => {
         ...
         return { ...prev, [activeScreenshotId]: { events: [...current.events, newEvent], annotations: [...current.annotations, newAnn] } };
       });
  };
